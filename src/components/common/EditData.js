...
//commit
  const fetchDeleteNotice = async (id, apiEndpoint, setDataFunction, fetchData) => {
    try {
        const response = await fetch(`${apiEndpoint}?_id=${id}`);
        ...
        if (response.ok) {
            ...
            await handleDeleteData('/api/deleteNotice', deleteId, fetchData);
        }
    } catch (error) {
        console.error(`Ошибка при получении данных: ${error}`);
    }
...
//commit
const fetchDeleteLeed = async (id, apiEndpoint, setDataFunction, fetchData) => {
  try {
      const response = await fetch(`${apiEndpoint}?_id=${id}`);
      ...
      if (response.ok) {
          ...
          // Выполнение удаления после установки данных
          const deleteId = {
              _id: data._id
          };
          await handleDeleteData('/api/deleteClientAll', deleteId, fetchData);
      }
  } catch (error) {
      console.error(`Ошибка при получении данных: ${error}`);
  }
  ...
  const registerClient = async (registrationData, fetchdata, upload, handleClientDoubleClick, setRegistrationDataClient, setAddLeedMode) => {
    try {
      ...
      if (response.ok) {
        alert('Registration successful. ');
        console.log(registrationData.status)
        fetchdata(registrationData.status)
        upload()
  ...
  export { fetchDeleteLeed, fetchDeleteNotice, fetchDataById, fetchDataByIdNotice, handleEditData, registerManager, registerClient, registerBuyer, registerProduct, formatDateTime, formatDateTimeTomorrow, formatDateTimeYet, handleDeleteData, registerNotice, registerTask, fetchDataByIdTask};