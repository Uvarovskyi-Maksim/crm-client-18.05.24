...
import { handleEditClient } from "../../../common/methods";
      ...
      .map((client) => (
        ...
        >
          ...
          className={`date_name_block ${
            new Date(client.selectedDate).setHours(0, 0, 0, 0) < new Date(currentDate).setHours(0, 0, 0, 0) ? "timeout" : ""
          } ${
            new Date(client.selectedDate).toDateString() === new Date(currentDate).toDateString() && (new Date(currentDate) - new Date(client.selectedDate)) >= 3600000 ? "uncompleted_tasks" : ""
          } ${
            new Date(client.selectedDate).toDateString() === new Date(currentDate).toDateString() && (new Date(currentDate) - new Date(client.selectedDate)) < 3600000 ? "today_tasks" : ""
          }`}          
          >
        ...
        <div style={{ color: "black" }} className="">
          ...
          <ul class="dropdown">
            ...
            {dataManagers.filter((person) => person.managerKey === keyManage || person.managerID === keyManage).map(manager => (
              <li class="dropdown-child" key={manager._id}>
                <label>
                  {manager.nameManager}
                  <input
                    type="checkbox"
